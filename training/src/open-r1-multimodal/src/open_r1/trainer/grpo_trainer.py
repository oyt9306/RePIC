    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                ...
                model = LlavaNextForConditionalGeneration.from_pretrained(model, **model_init_kwargs)
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                self.ref_model = LlavaNextForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
            elif "Aria" in model_id:
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2.5-VL" in model_id or "Aria" in model_id:
                ...
                processing_class = LlavaNextProcessor.from_pretrained(model_id)
                ...
                for im in img:
                    ...
                    if w > 512 or h > 512 :
                        if w < h:
                            ...
                            new_h = 512
                            new_w = int(w * (512/h))
                        im = im.resize((new_w, new_h), PIL.Image.Resampling.LANCZOS)
                    imgs.append(im)
                images.append(imgs)
            else:
                w, h = img.size
