...
def run_captioning(modelname, dataset_name, retrieval_mode, topK, index_path, device):
    ...
    if modelname == 'RePIC':
        ...
        QUESTION_TEMPLATE = "{Question}"
    elif modelname == 'RAP_Qwen':
        MODEL_PATH = 'Yeongtak/RAP_Qwen2.5VL_7B'
        QUESTION_TEMPLATE = "{Question}"
    ...
    # Target dataset
    if dataset_name == 'myvlm':
        database_path = './data/database_tot/database_myvlm'
        eval_file = './data/query_tot/myvlm_query_templates.json'
    elif dataset_name == 'dreambooth':
        database_path = './data/database_tot/database_dreambooth'            
        eval_file = './data/query_tot/dreambooth_query_templates.json'
    elif dataset_name == 'yollava':
        database_path = './data/database_tot/database_yollava'
        eval_file = './data/query_tot/yollava_query_templates.json'
    else:
        raise ValueError 
    ...
    save_path = f"./save_script/single_captioning-retrieval-{retrieval_mode}-{dataset_name}-{modelname}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Single-concept Image Captioning Evaluation")
    parser.add_argument("--modelname", type=str, default="RePIC", choices=["RePIC", "RePIC_huggingface", "RAP_Qwen", "zero_shot"])
    parser.add_argument("--datasetname", type=str, default="yollava", choices=["yollava", "myvlm", "dreambooth"])
